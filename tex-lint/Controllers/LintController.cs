        ...
        [FromForm] string? startFile = null)
    {
        try
        {
            ...
            try
            {
                ...
                testUtilities.SetFoundsCommands(requestId, commands);
                testUtilities.SetFoundsCommandsWithLstlisting(requestId, commands);

                // Запускаем тестовые функции для проверки
                var testHandler = new TestFunctionClasses.TestFunctionHandler(_configurationService, requestId);
    ...
    public IActionResult CheckLatex([FromBody] LatexCheckRequest request)
    {
        try
        {
            ...
            try
            {
                ...
                var testFunctionHandler = new TestFunctionClasses.TestFunctionHandler(_configurationService, requestId);
